...
public interface UserRepository extends JpaRepository<User, String> {
  ...
  @Query(
      "SELECT u FROM User u WHERE (LOWER(u.login) LIKE LOWER(CONCAT('%', :login, '%')) OR"
          + " LOWER(u.firstName) LIKE LOWER(CONCAT('%', :login, '%')) OR LOWER(u.lastName) LIKE"
          + " LOWER(CONCAT('%', :login, '%'))) AND u.password IS NOT NULL AND LENGTH(u.password) >"
          + " 0 AND u.team IS NOT NULL AND u.role IS NOT NULL")
  ...
  @Query(
      "SELECT u FROM User u WHERE (LOWER(u.login) LIKE LOWER(CONCAT('%', :login, '%')) OR"
          + " LOWER(u.firstName) LIKE LOWER(CONCAT('%', :login, '%')) OR LOWER(u.lastName) LIKE"
          + " LOWER(CONCAT('%', :login, '%'))) AND u.team.id = :team_id")
