...
public class RoleService {
  ...
  public List<Status> getStatuses(Integer id) {
    List<RoleStatus> roleStatuses =
        roleStatusRepository.findAllByRole_Id(id, Sort.by(Sort.Direction.ASC, "status_id"));
    return roleStatuses.stream().map(RoleStatus::getStatus).collect(Collectors.toList());
  }
