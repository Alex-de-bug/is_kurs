...
public class TaskService {
  ...
  public Page<Task> getFilteredTasks(
      Integer statusId, Integer sprintId, User implementer, Integer tagId, int page) {

    Specification<Task> spec =
        Specification.where(TaskSpecification.hasStatus(statusId))
  ...
  public Task createTask(TaskDto taskDto, User user) {
    ...
    task.setStatus(
        statusRepository
            .findById(1)
            .orElseThrow(() -> new NoSuchElementException("Стандартный статус не найден")));
  ...
  public boolean canEditTask(User user, Task task) {
    ...
    return user.getLogin().equals(task.getCreatedBy().getLogin())
        || (task.getImplementer() != null
            && user.getLogin().equals(task.getImplementer().getLogin()));
  }
  ...
  public Task updateStatus(Integer taskId, Integer statusId, User user) {
    ...
    } else if (task.getImplementer() != null
        && task.getImplementer().getLogin().equals(user.getLogin())
        && user.getRole() != null) {
      List<Status> allowedStatuses = roleService.getStatuses(user.getRole());
      Optional<Status> allowedStatus =
