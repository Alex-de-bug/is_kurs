...
public class TagService {
  ...
  public Tag createTag(Tag tag) {
    ...
    if (tag.getDescription() != null) tag.setDescription(tag.getDescription().trim());
    tagRepository
  ...
  public Tag updateTag(Integer id, Tag updatedTag) {
    Tag tag =
        tagRepository.findById(id).orElseThrow(() -> new NoSuchElementException("Тег не найден"));

    if (!tag.getName().equals(updatedTag.getName().trim())) {
      tagRepository
          .findByName(updatedTag.getName().trim())
          .ifPresent(
              existingTag -> {
                throw new NoSuchElementException("Тег с таким названием уже существует");
              });
    ...
    if (updatedTag.getDescription() != null) tag.setDescription(updatedTag.getDescription().trim());
